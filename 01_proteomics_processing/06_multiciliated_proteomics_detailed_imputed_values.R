...
ctrl <- read.csv("mccs_ctrl_b2_b5.csv")
il13 <- read.csv("mccs_il-13_f17_f20.csv")
...
analyze_protein <- function(ctrl_vals, il13_vals) {
  ...
  if (length(observed_values) < 2) {
    return(c(
      ...
      Log2.Difference = NA,
      p.value = NA,
      ctrl_1 = NA, ctrl_2 = NA, ctrl_3 = NA,
      il13_1 = NA, il13_2 = NA, il13_3 = NA,
      ctrl_imp_1 = NA, ctrl_imp_2 = NA, ctrl_imp_3 = NA,
      il13_imp_1 = NA, il13_imp_2 = NA, il13_imp_3 = NA
    ))
  }
...
results_mtrx <- matrix(NA, nrow = nrow(ctrl), ncol = 16)
...
for (x in 1:nrow(ctrl)) {
  ctrl_vals <- as.numeric(ctrl[x, 5:7])
  il13_vals <- as.numeric(il13[x, 5:7])
...
write_xlsx(results_detailed_df, "mccs_imputed_detailed_results.xlsx")
write.csv(results_detailed_df, "mccs_imputed_detailed_results.csv", row.names = FALSE)
